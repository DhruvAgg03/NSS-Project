...
Universe::Universe(int maxR=5,int *organismCount, biodata_Plant variety_Plant[varieties_in_a_Species] = {{10},{20},{30}}, biodata_Insect variety_Insect[varieties_in_a_Species] = {{10,3,300},{20,2,200},{30,1, 100}})
{
...
void Universe::initializeVarieties(biodata_Plant* plantVarieties, biodata_Insect* insectVarieties){
    ...
    // The memory address initially stored in the pointers are the same the vakues it points are different 
}
    ...
    for(int i=0;i<N;i++)
    {
        for(int j=0;j<organismCount[i];j++)
        {
            if(i==0){
                ...
                namespace_organism::Plant temp(std::get<0>(tempPoints[counter]), std::get<1>(tempPoints[counter]), 0, NULL,0, variety_Plant[variety].max_energy, variety_Plant[variety].max_energy, counter, 0);
